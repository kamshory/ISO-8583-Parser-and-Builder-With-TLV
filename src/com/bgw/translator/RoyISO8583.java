...
public class RoyISO8583 
{
	 ...
	 */
	private static JSONObject generalConfig = new JSONObject();;
	/**
	...
	public String getBody()
	{
		...
		for(iter = 0; iter < this.fields.size(); iter++)
		{
			fieldInt = this.fields.get(iter);
			jo = (JSONObject) this.item.get("f"+fieldInt);
			try
			{
	...
	public String getValue(int field)
	{
		...
		try
		{
			jo = (JSONObject) this.item.optJSONObject("f"+field);
			if(jo != null)
			{
	...
	public void parse(String message)
	{
		...
		for(iter = 0; iter<this.fields.size(); iter++)
		{
			...
			if(jo == null)
			{
				...
				if(RoyISO8583.generalConfig != null)
				{
					jo = (JSONObject) RoyISO8583.generalConfig.optJSONObject("f"+field);
					if(jo == null)
					{
						validMessage = false;
					}
