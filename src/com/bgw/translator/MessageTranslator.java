...
public class MessageTranslator extends Translator{
	...
	public JSONObject parseISO8583(String message, JSONObject formatMap)
	{
		...
		JSONObject tlvData = new JSONObject();
					...
					try
					{
						...
						for(idx = 2; idx <= maxField; idx++)
						{
							...
							if(royISO8583.hasField(idx))
							{
								...
								{
									String jostr = formatMap.optString(fieldName, "");								
									try 
									{
										fieldFormat = new JSONObject(jostr);
										format = fieldFormat.optString("format", "");
										variable = fieldFormat.optString("variable", "");
										isTLV = fieldFormat.optBoolean("tlv", false);
										...
										if(isTLV)
										{
											tags = fieldFormat.optJSONArray("tags");
											tlvData = TLV.parse(subfield, tags);
											Iterator<String> keys = tlvData.keys();

											while(keys.hasNext()) 
											{
											    String keyX = keys.next();
											    String valX = tlvData.optString(keyX, "");
											    json.put(keyX, valX);
											}
										}
										else
		...
		{
			System.out.println("Unable to parse message because format map is empty. Please check spec.");
		}
		return json;
	}	
	/**
	 ...
	 */
	public String buildISO8583(JSONObject json, JSONArray formatMap, String mti_id)
	{
	 ...
	 * @return Return byte contains ISO 8583 message
	 */
	public String buildISO8583(JSONObject json, JSONObject formatMap, String mti_id)
	{
		...
		try 
		{
			try
			{
			    ...
			    do
			    {
			        ...
			        {			        	
				        row = formatMap.optJSONObject(key);		
				        if(row == null)
				        {
				        	row = new JSONObject();
				        }
				        format = row.optString("format", "");
				        variable = row.optString("variable", "");
				        variable = variable.trim();
				        field_length = row.optInt("field_length", 0);
				        options = row.optString("options", "");
				        field_type = row.optString("type", "");
				        
				        isTLV = row.optBoolean("tlv", false);
				        ...
				        if(variable.length() > 0)
				        {
					        ...
					        else
					        {
					        	...
					        	{
					        		subfield = json.optString(variable, "");
					        	}
					        	else
					        	{
					        		subfield = "";
				        ...
				        if(isTLV)
				        {
				        	...
				        	data = TLV.build(json, tags);
				        	royISO8583.setValue(field, data, field_type, data.length());
				        }
			        }
			    }
			    while(iter.hasNext());
	...
	public Map<String, String> toMapString(JSONObject object) throws JSONException 
	{
	...
	public byte[] buildRoyISO8583(JSONObject json, JSONObject formatMap, String mti_id)
	{
		...
		{
			System.out.println("Unable to build message because format map is empty. Please check spec.");
		}
		return message;
	}
	/**
